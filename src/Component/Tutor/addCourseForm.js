  ...
  const onSubmit = (e) => {
    ...
    courseData.set(
      ...
      `${course.courseName
        ...
        .trim()
        .replaceAll(' ', '')
        .replace(/[^a-zA-Z ]/g, '')}_banner_image`
    );

    courseData.set('courseName', course.courseName);
    courseData.set('category', course.category);
    courseData.set('institute', course.institute);
    courseData.set('instructor', course.instructor);
    courseData.set('durationInWeaks', course.durationInWeaks);
    courseData.set('hasOnDemandVideo', course.hasOnDemandVideo);
    courseData.set('hasDownloadableResources', course.hasDownloadableResources);
    courseData.set('hasLifeTimeAccess', course.hasLifeTimeAccess);
    courseData.set('hasCertification', course.hasCertification);
    courseData.set('language', course.language);
    courseData.set('numLectures', course.numLectures);
    courseData.set('overViewText', course.overViewText);
    courseData.set('minPrice', course.minPrice);
    courseData.set('maxPrice', course.maxPrice);
    courseData.set('batchName', batch.batchName);
    courseData.set('batchMaxStudents', batch.maxStudents);
    courseData.set('batchStartDate', batch.startDate);
    courseData.set('batchStartTime', batch.startTime);
    courseData.set('batchEndDate', batch.endDate);
    courseData.set('batchEndTime', batch.endTime);
              ...
              <div id='select-chapter-dropdown'>
                {chapters.map((n) => (
                  <Chapter
                    num={n + 1}
                    chapter={chapter}
                    inputChange={(file) => handleDocInput(file)}
                    onInputChapter={(e) => {
                      onInputChapter(e);
                    }}
                  />
